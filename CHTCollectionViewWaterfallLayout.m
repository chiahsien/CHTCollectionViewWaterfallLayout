...
- (void)prepareLayout {
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        for (idx = 0; idx < itemCount; idx++) {
            ...
            NSUInteger columnIndex = [self nextColumnIndexForItem:idx inSection:section totalItems:itemCount];
 ...
 */
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section totalItems:(NSInteger)numItems {
    NSUInteger index = 0;
    NSInteger columnCount = [self columnCountForSection:section];
    switch (self.itemRenderDirection) {
        ...
        case CHTCollectionViewWaterfallLayoutItemRenderDirectionEqualColumns:
        {
            NSInteger items_per_column = ceil((float)numItems / (float)columnCount);
            index = (item / items_per_column);
        }
            break;
